...
- OperatorOS SSO destination at `/sso` implements the canonical Shotgun Ninjas Child-App SSO Integration contract: HS256 verification, 90-second token age + 5-second clock-skew rules, mandatory `/v1/modules/sso/consume` call, sub-keyed local user records (with email backfill for users provisioned before the contract), and content-negotiated reject responses (HTML for browsers, JSON `{code}` for `Accept: application/json`). Env vars: `MODULE_SSO_SECRET`, `OPERATOROS_BASE_URL`, `OPERATOROS_SSO_AUDIENCE`, `OPERATOROS_SSO_ENV` (`prod`/`staging`/`dev`), `OPERATOROS_API_URL` (defaults to base url). Legacy `MODULE_SLUG` / `APP_ENV` still accepted. Production refuses to start without `MODULE_SSO_SECRET`; dev boots fine and `/sso` returns a clean 503 "not configured" page. See `docs/OPERATOROS_SSO_DESTINATION.md` for the full contract.
